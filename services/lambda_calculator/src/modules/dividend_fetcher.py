...
def dividend_fetcher(df_investment_info: pl.DataFrame, connection_config: dict[str, str]) -> pl.DataFrame:
    try:
        ...
        df_dividend_pd: pd.DataFrame = pd.read_sql_query(select_query, engine)
        df_dividend_pd["investment_code"] = df_dividend_pd["investment_code"].astype(str)
        df_dividend: pl.DataFrame = pl.DataFrame(df_dividend_pd)
